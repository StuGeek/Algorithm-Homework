...
```
 ...
 在实现中使用LinkedList中定义的类型Ptr时，需要使用限定符::说明它所在的类。在有的编译器中还需要说明它是一个类型。例如

```
 template <typename T> LinkedList<T> :: Ptr  LinkedList<T> :: find( T &x) const{
  ...
 }
```
 或者
...
```
 template <typename T> typename LinkedList<T> :: Ptr  LinkedList<T> :: find( T &x) const{
  ...
 }
```
